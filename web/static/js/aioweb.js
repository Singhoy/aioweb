...
function toSmartDate(timestamp) {
    ...
    } else if (t >= (60 * 60 * 24 * 10000)) {
        // 一到六天前
        s = Math.floor(t / (60 * 60 * 24 * 1000)) + '天前';
    } else if (t >= (60 * 60 * 1000)) {
        // 一天内
        s = Math.floor(t / (60 * 60 * 1000)) + '小时前';
    } else if (t >= (60 * 1000)) {
    ...
    $form.submit(function (event) {
        ...
        postJSON($form.attr('action-url'), data, function (err, result) {
            ...
            if (err) {
                ...
                const r = fn_success ? window[fn_success](result) : false;
                if (r === false) {
                    $submit.removeAttr('disabled');
                }
            }
...
function _display_error($obj, err) {
    ...
    let L = ['<div class="uk-alert uk-alert-danger">'];
    L.push('<p>Error: ');
    L.push(msg);
    L.push('</p><p>Code: ');
